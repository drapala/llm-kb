...
| [[reflexion-weighted-knowledge-graphs]] vs [[agent-memory-architectures]] (Zep/Hindsight/RMM) | "Modify edge weights via verbal reflection" vs "Temporal invalidation (Zep), opinion reinforcement (Hindsight), online RL (RMM)" | Mechanism-contingent: RWKG proposed ad-hoc weight modification; prior work uses more principled mechanisms. Zep preserves temporal history, Hindsight separates facts from beliefs structurally, RMM uses formal RL. RWKG concept is partially subsumed by these implementations. |
